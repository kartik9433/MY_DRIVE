...
function Drive() {
  ...
  const fetchFiles = async () => {
    try {
      const response = await fetch(`https://${process.env.REACT_APP_API_URL}/apis/files/list`);
      if (response.ok) {
  ...
  const fetchfiles = async (file) => {
    ...
    try {
      const response = await fetch(`https://${process.env.REACT_APP_API_URL}/apis/files/upload`, {
        method: "POST",
        body: formData,
      });
  ...
  const downloadFile = async (file) => {
    try {
      const response = await fetch(`https://${process.env.REACT_APP_API_URL}/apis/files/download/${file.id}`, {
        method: "GET",
      });
      if (!response.ok) {
  ...
  const deleteFile = async (file) => {
    try {
      const response = await fetch(`https://${process.env.REACT_APP_API_URL}/apis/files/delete/${file.id}`, {
        method: "DELETE",
      });
